  ...
  pageShell(`
    <header class="admin-header">
      ...
      <div class="header-actions" aria-label="Outils du compte">
        ...
        <span class="admin-version" aria-label="Version v1.23.11">v1.23.11</span>
      </div>
    </header>
    <nav class="admin-nav" aria-label="Administration">
      <button class="is-active" type="button" data-panel-button="drafts" aria-pressed="true"><span class="admin-nav-index">01</span><span class="admin-nav-label">Brouillons</span><span class="admin-nav-count" id="draft-count">0</span></button>
      <button type="button" data-panel-button="publish" aria-pressed="false"><span class="admin-nav-index">02</span><span class="admin-nav-label">Publier</span><span class="admin-nav-count" id="selected-count">0</span></button>
      <button type="button" data-panel-button="publications" aria-pressed="false"><span class="admin-nav-index">03</span><span class="admin-nav-label">Publications</span></button>
      <button type="button" data-panel-button="links" aria-pressed="false"><span class="admin-nav-index">04</span><span class="admin-nav-label">Liens publiés</span></button>
      <button type="button" data-panel-button="editions" aria-pressed="false"><span class="admin-nav-index">05</span><span class="admin-nav-label">Éditions</span></button>
      <button type="button" data-panel-button="categories" aria-pressed="false"><span class="admin-nav-index">06</span><span class="admin-nav-label">Catégories</span></button>
    ...
    <section class="admin-panel" data-panel="themes">
      <div class="section-heading">
        <div><p class="kicker">Registre éditorial</p><h2>Tags</h2><p class="intro">Crée, documente et organise le vocabulaire proposé dans l’extension et les brouillons. Les routes archivées restent conservées.</p></div>
        <div class="tag-register-stats" aria-label="État du registre">
          <span><strong id="theme-count">0</strong> actifs</span>
          <span><strong id="theme-archived-count">0</strong> archivés</span>
          <span><strong id="theme-undocumented-count">0</strong> à documenter</span>
        </div>
      </div>
      <form id="theme-create-form" class="theme-create-form">
        ...
        <label>Description<input name="description" maxlength="500" placeholder="Périmètre éditorial du tag"></label>
        <label>Alias<input name="aliases" maxlength="500" placeholder="Variantes séparées par des virgules"></label>
        <button class="primary" type="submit">Ajouter le tag</button>
      </form>
      <div class="tag-register-tools">
        <div class="tag-register-views" aria-label="Filtrer les tags">
          <button class="is-active" type="button" data-theme-view="active" aria-pressed="true">Actifs</button>
          <button type="button" data-theme-view="archived" aria-pressed="false">Archivés</button>
          <button type="button" data-theme-view="undocumented" aria-pressed="false">À documenter</button>
        </div>
        <input id="theme-search" type="search" placeholder="Nom, description ou alias" aria-label="Rechercher un tag par nom, description ou alias">
      </div>
...
h1{max-width:900px;margin:.35rem 0 .75rem;font-family:Arial,Helvetica,sans-serif;font-size:clamp(2.8rem,7vw,5.25rem);letter-spacing:-.055em;line-height:.88;text-wrap:balance}
h2{margin:.25rem 0;font-family:Arial,Helvetica,sans-serif;font-size:clamp(2rem,4.5vw,3.75rem);letter-spacing:-.045em;line-height:.92;text-wrap:balance}
.intro{max-width:680px;color:var(--muted);font-size:1rem;line-height:1.7;text-wrap:pretty}
.admin-heading .intro{margin:0;font-size:.84rem;line-height:1.55}
button,a,input,select,textarea{font:inherit}
button,a{border:1px solid var(--line);border-radius:0;background:transparent;color:inherit;cursor:pointer;font-weight:700;text-decoration:none}
button{min-height:44px;padding:.65rem .8rem}
...
.header-actions{display:grid;min-width:11.5rem;grid-template-rows:auto auto auto 1fr;border-left:1px solid var(--line)}
.header-actions a,.header-actions button{display:flex;min-height:42px;align-items:center;justify-content:space-between;gap:1rem;padding:.55rem .7rem;border:0;border-bottom:1px solid var(--line);font-size:.7rem;letter-spacing:.06em;text-align:left;text-transform:uppercase}
.header-actions a:hover,.header-actions button:hover{background:var(--ink);color:var(--paper)}
.header-actions a span:last-child{color:var(--accent-text)}
.admin-version{display:flex;align-items:flex-end;padding:.55rem .7rem;background:var(--ink);color:var(--paper);font-size:.66rem;font-weight:700;letter-spacing:.08em}
.toolbar,.form-actions{display:flex;flex-wrap:wrap;gap:0}
.toolbar>*+*,.form-actions>*+*{margin-left:-1px}
.toolbar button,.toolbar input,.toolbar select{min-height:40px;height:40px;padding:.45rem .65rem;font-size:.74rem;font-weight:400}
.draft-toolbar{display:grid;grid-template-columns:max-content minmax(13rem,1fr) minmax(7rem,8rem);width:500px!important;max-width:100%}
.feedback{min-height:1.5em;margin:.65rem 0;color:var(--accent-text);font-size:.78rem}
.feedback:empty{display:none}
.admin-nav{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:1px;background:var(--line);border:0 solid var(--line);border-width:0 1px 1px}
.admin-nav button{display:grid;grid-template-columns:1.7rem minmax(0,1fr) auto;min-width:0;min-height:52px;align-items:center;gap:.45rem;padding:.55rem .65rem;border:0;border-radius:0;background:var(--paper);font-size:.74rem;text-align:left;white-space:normal}
.admin-nav button:hover{background:#ebe7df}
.admin-nav button.is-active{background:var(--ink);color:var(--paper)}
.admin-nav-index{color:var(--muted);font-size:.6rem;font-weight:400;letter-spacing:.08em}
.admin-nav-label{overflow-wrap:anywhere}
...
.toolbar input,.toolbar select{width:auto;min-width:220px;flex:1 1 220px}
.section-heading .toolbar{width:min(100%,620px);justify-self:end}
.draft-toolbar input,.draft-toolbar select{min-width:0}
...
.category-actions{display:flex;flex-wrap:wrap;gap:0}
.category-actions>*+*,.theme-actions>*+*{margin-left:-1px}
.danger{color:var(--error)}
.tag-register-stats{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));min-width:22rem;border:1px solid var(--line)}
.tag-register-stats span{display:grid;gap:.15rem;padding:.65rem .8rem;border-left:1px solid var(--line);color:var(--muted);font-size:.68rem;text-transform:uppercase}
.tag-register-stats span:first-child{border-left:0}
.tag-register-stats strong{font-family:Arial,Helvetica,sans-serif;font-size:1.7rem;line-height:1;color:var(--ink)}
.tag-register-tools{display:flex;justify-content:space-between;gap:1rem;margin-bottom:1rem}
.tag-register-tools>input{max-width:28rem}
.tag-register-views{display:flex;gap:1px;background:var(--line);border:1px solid var(--line)}
.tag-register-views button{border:0;border-radius:0;background:var(--paper)}
.tag-register-views button.is-active{background:var(--ink);color:var(--paper)}
...
.theme-row{display:grid;grid-template-columns:.65fr 1.2fr 1fr minmax(12rem,auto);align-items:end;gap:.7rem;border-top:1px solid var(--line);padding:1rem 0}
.theme-row.is-filtered{display:none}
.theme-actions{display:flex;flex-wrap:wrap;justify-content:flex-end;gap:0}
.theme-usage{margin:.4rem 0 0;color:var(--muted);font-size:.68rem}
.theme-state{color:var(--muted);font-size:.68rem;font-weight:700;text-transform:uppercase}
.theme-state.is-undocumented{color:var(--warn)}
.theme-row-error{grid-column:1/-1;margin:0;color:var(--error);font-size:.72rem}
...
.card-actions{display:flex;flex-wrap:wrap;justify-content:space-between;align-items:center;gap:.5rem;margin-top:1rem}
...
.publication-links{display:flex;flex-wrap:wrap;gap:0;margin:.8rem 0 0}
.publication-links>*+*{margin-left:-1px}
...
.status{display:inline-block;padding:.25rem .5rem;border:1px solid var(--line);border-radius:0;background:#e7e2d8;font-size:.72rem}
...
let tagRegister=[];
let themeView="active";
const foldTheme=(value)=>String(value||"").normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLocaleLowerCase("fr");
const themeErrorLabel=(error)=>({INVALID_THEME_NAME:"Saisissez un nom de tag valide.",INVALID_THEME:"Vérifiez le nom et les alias.",THEME_ALREADY_EXISTS:"Ce nom ou cet alias existe déjà.",THEME_RESERVED:"Ce nom appartient à un tag archivé.",THEME_NOT_FOUND:"Ce tag n’existe plus."}[error.message]||error.message);
const themeState=(theme)=>theme.active!==false&&!theme.description.trim()?'<span class="theme-state is-undocumented">À documenter</span>':theme.active!==false?'<span class="theme-state">Actif</span>':'<span class="theme-state">Archivé</span>';
const renderThemes=(themes=tagRegister)=>{const target=document.querySelector("#theme-list");if(!target)return;tagRegister=themes;const active=themes.filter((theme)=>theme.active!==false),archived=themes.filter((theme)=>theme.active===false),undocumented=active.filter((theme)=>!theme.description.trim());document.querySelector("#theme-count").textContent=String(active.length);document.querySelector("#theme-archived-count").textContent=String(archived.length);document.querySelector("#theme-undocumented-count").textContent=String(undocumented.length);const mergeOptions=(current)=>'<option value="">Fusionner vers…</option>'+active.filter((theme)=>theme.name!==current).map((theme)=>'<option value="'+esc(theme.name)+'">'+esc(theme.name)+'</option>').join("");target.innerHTML=themes.map((theme)=>{const disabled=theme.active===false?' disabled':'';const actions=theme.active===false?'<button type="button" data-reactivate-theme>Réactiver</button>':'<button type="button" data-save-theme>Enregistrer</button><select name="mergeTarget" aria-label="Tag cible">'+mergeOptions(theme.name)+'</select><button type="button" data-merge-theme>Fusionner</button><button class="danger" type="button" data-archive-theme>Archiver</button>';return '<article class="theme-row" data-theme-row data-theme-active="'+String(theme.active!==false)+'" data-theme-undocumented="'+String(theme.active!==false&&!theme.description.trim())+'" data-theme-name="'+esc(theme.name)+'" data-theme-search-text="'+esc(foldTheme(theme.name+" "+theme.description+" "+theme.aliases.join(" ")))+'"><label>Nom<input name="themeName" maxlength="80" value="'+esc(theme.name)+'"'+disabled+'></label><label>Description<input name="themeDescription" maxlength="500" value="'+esc(theme.description)+'"'+disabled+'></label><label>Alias<input name="themeAliases" maxlength="500" value="'+esc(theme.aliases.join(", "))+'"'+disabled+'><span class="theme-usage">'+(theme.linkCount??0)+' lien(s) publié(s) · '+(theme.draftCount??0)+' brouillon(s)</span>'+themeState(theme)+'</label><div class="theme-actions">'+actions+'</div><p class="theme-row-error" data-theme-error role="status"></p></article>'}).join("");filterThemes()};
const filterThemes=()=>{const query=foldTheme(document.querySelector("#theme-search")?.value||"");document.querySelectorAll('[data-theme-row]').forEach((row)=>{const viewMatch=themeView==="archived"?row.dataset.themeActive==="false":themeView==="undocumented"?row.dataset.themeUndocumented==="true":row.dataset.themeActive==="true";row.classList.toggle("is-filtered",!viewMatch||Boolean(query)&&!row.dataset.themeSearchText.includes(query))})};
document.querySelector("#theme-search")?.addEventListener("input",filterThemes);
document.querySelectorAll("[data-theme-view]").forEach((button)=>button.addEventListener("click",()=>{themeView=button.dataset.themeView;document.querySelectorAll("[data-theme-view]").forEach((candidate)=>{const active=candidate===button;candidate.classList.toggle("is-active",active);candidate.setAttribute("aria-pressed",String(active))});filterThemes()}));
const loadThemes=async()=>{const data=await api("/api/admin/themes");tagRegister=data.themes;options.themes=data.themes.filter((theme)=>theme.active!==false);options.tags=options.themes.map((theme)=>theme.name);renderThemes(data.themes);return data.themes};
...
const taxonomyMutationRequest=(parts)=>{const key="digest-taxonomy-mutation:"+JSON.stringify(parts);let requestId=localStorage.getItem(key);if(!requestId){requestId=crypto.randomUUID();localStorage.setItem(key,requestId)}return{requestId,complete:()=>localStorage.removeItem(key)}};
document.querySelector("#theme-create-form")?.addEventListener("submit",async(event)=>{event.preventDefault();const form=event.currentTarget,button=event.submitter;button.disabled=true;try{const data=await api("/api/admin/themes",{method:"POST",body:JSON.stringify({name:form.elements.name.value,description:form.elements.description.value,aliases:themeAliases(form.elements.aliases.value),confirm:true})});form.reset();themeView=data.theme.description?"active":"undocumented";await refreshTaxonomy();show(data.changed?"Tag ajouté. Un nouveau déploiement est lancé.":"Ce tag existait déjà ; il a été affiché dans le registre.")}catch(error){show("Tag impossible à ajouter : "+themeErrorLabel(error))}finally{button.disabled=false}});
document.querySelector("#theme-list")?.addEventListener("click",async(event)=>{const action=event.target.closest('[data-save-theme],[data-merge-theme],[data-archive-theme],[data-reactivate-theme]');if(!action)return;const row=action.closest('[data-theme-name]'),current=row.dataset.themeName,errorTarget=row.querySelector('[data-theme-error]');action.disabled=true;errorTarget.textContent="";try{let data;if(action.matches('[data-reactivate-theme]')){if(!confirm('Réactiver le tag « '+current+' » dans les suggestions ?'))return;data=await api("/api/admin/themes/"+encodeURIComponent(current)+"/reactivate",{method:"POST",body:JSON.stringify({confirm:true})});themeView="active";await refreshTaxonomy();show("Tag réactivé · "+data.preservedLinks+" lien(s) publié(s) conservé(s).") }else if(action.matches('[data-archive-theme]')){const source=tagRegister.find((theme)=>theme.name===current);if(!confirm('Archiver le tag « '+current+' » ? '+(source?.linkCount||0)+' lien(s) publié(s) seront conservés et le tag sera retiré de '+(source?.draftCount||0)+' brouillon(s).'))return;const request=taxonomyMutationRequest(["archive-theme",current]);data=await api("/api/admin/themes/"+encodeURIComponent(current),{method:"DELETE",body:JSON.stringify({confirm:true,requestId:request.requestId})});request.complete();themeView="archived";await refreshTaxonomy();show("Tag archivé · "+data.preservedLinks+" lien(s) conservé(s) · retiré de "+data.removedDrafts+" brouillon(s).") }else{const mergeTarget=row.querySelector('[name="mergeTarget"]')?.value,replacement=action.matches('[data-merge-theme]')?mergeTarget:row.querySelector('[name="themeName"]').value,description=row.querySelector('[name="themeDescription"]').value,aliases=themeAliases(row.querySelector('[name="themeAliases"]').value);if(!replacement){errorTarget.textContent="Choisissez un tag cible pour la fusion.";return}if(action.matches('[data-merge-theme]')&&!confirm('Fusionner « '+current+' » avec « '+replacement+' » ? Les liens et brouillons seront migrés.'))return;const request=taxonomyMutationRequest(["update-theme",current,replacement,description,aliases]);data=await api("/api/admin/themes/"+encodeURIComponent(current),{method:"PATCH",body:JSON.stringify({name:replacement,description,aliases,confirm:true,requestId:request.requestId})});request.complete();await refreshTaxonomy();show((data.merged?"Tags fusionnés":"Tag enregistré")+" · "+data.migrated+" lien(s) et "+data.migratedDrafts+" brouillon(s) migré(s).")}}catch(error){errorTarget.textContent=themeErrorLabel(error)}finally{action.disabled=false}});
