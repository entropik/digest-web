...
test("paragraph and list texts are justified with hyphenation and left-aligned last lines", async () => {
  const css = await readFile(cssPath, "utf8");

  assert.match(
    css,
    /^p,\s*li\s*\{[^}]*text-align:\s*justify;[^}]*text-align-last:\s*left;[^}]*text-justify:\s*inter-word;/ms,
  );
  assert.match(
    css,
  ...
  assert.match(
    css,
    /\.archive-intro\s+li\s*\{[^}]*text-align:\s*justify;[^}]*text-align-last:\s*left;/s,
  );
  assert.match(
    css,
