...
document.querySelector("#published-links")?.addEventListener("click",async(event)=>{const button=event.target.closest("[data-save-link]");if(!button)return;const card=button.closest("[data-link-id]");const urlInput=card.querySelector('[name="url"]');if(!urlInput.checkValidity()){urlInput.reportValidity();return}button.disabled=true;try{const body={url:urlInput.value,title:card.querySelector('[name="title"]').value,category:card.querySelector('[name="category"]').value,description:card.querySelector('[name="description"]').value,tags:card.querySelector('[name="tags"]').value.split(",").map((tag)=>tag.trim()).filter(Boolean),confirm:true};const data=await api("/api/admin/links/"+encodeURIComponent(card.dataset.linkId),{method:"PATCH",body:JSON.stringify(body)});urlInput.value=data.link.url;card.querySelector("[data-link-title]").textContent=data.link.title;show(data.changed?"Lien corrigé. Un nouveau déploiement est lancé.":"Aucune modification à enregistrer.")}catch(error){show("Correction impossible : "+correctionErrorLabel(error.message))}finally{button.disabled=false}});
