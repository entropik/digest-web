...
test("Hugo English adapter reuses current fields, falls back after edits and preserves equivalent routes",async()=>{
  ...
  try {
    ...
    for(const file of ["content/_content.en.gotmpl","i18n/en.json","i18n/fr.json","layouts/partials/english-content.html","layouts/partials/public-route-map.html","layouts/partials/about-liquid-script.html","layouts/partials/archive-og-image-alt.html","layouts/partials/taxonomy-translation-pending.html","layouts/partials/translated-link.html","layouts/partials/translated-visual.html","layouts/partials/translated.html","layouts/partials/translation-field.html","layouts/partials/translation-status.html","static/js/about-liquid.js"]){
    ...
    await writeFile(path.join(directory,"data/categories.json"),JSON.stringify([{name:"IA",description:""}]));
    await writeFile(path.join(directory,"data/tags.json"),JSON.stringify([{name:"veille",description:""}]));
    await writeFile(path.join(directory,"layouts/single.html"),'<html lang="{{ site.Language.Lang }}"><h1>{{ .Title }}</h1><p>{{ .Params.translation_pending }}</p><p>{{ index .Params.images 0 }}|{{ .Params.visual }}|{{ .Params.translation_artwork_current }}</p>{{ $link := partial "translated-link.html" (dict "id" "fixture" "title" "Titre du lien") }}<p>link={{ $link.title }}|{{ $link.translation_pending | default false }}</p>{{ $visual := partial "translated-visual.html" (dict "image" "/poster.png" "title" "Affiche") }}<p>visual={{ $visual.title }}|{{ $visual.translationPending }}</p>{{ $pendingVisual := partial "translated-visual.html" (dict "image" "/pending.png" "title" "En attente") }}<p>pending-visual={{ $pendingVisual.title }}|{{ $pendingVisual.translationPending }}</p><p>taxonomy-current={{ partial "taxonomy-translation-pending.html" (dict "categories" (slice "IA")) }}</p><p>taxonomy-pending={{ partial "taxonomy-translation-pending.html" (dict "tags" (slice "veille")) }}</p><meta name="fixture-og-alt" content="{{ partial "archive-og-image-alt.html" .Title }}">{{ .Content }}{{ partial "about-liquid-script.html" . }}</html>');
    ...
    assert.match(html,/link=Link title\|false/);assert.match(html,/visual=Poster\|false/);assert.match(html,/pending-visual=En attente\|true/);assert.match(html,/taxonomy-current=false/);assert.match(html,/taxonomy-pending=true/);assert.match(html,/content="Generative poster for Digest edition Titre changé"/);
