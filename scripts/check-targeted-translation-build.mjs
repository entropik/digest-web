...
import { segmentConfig } from "./translation-production.mjs";
...
try {
  ...
  await writeFile(config, segmentConfig(["/a-propos/"]));
  const result = spawnSync(process.env.HUGO_BINARY || "hugo", ["--gc", "--minify", "--panicOnWarning", "--baseURL", "https://digest.ooblik.com/", "--config", `hugo.yaml,${config}`, "--renderSegments", "translation-target,translation-snapshot", "--destination", destination, "--cleanDestinationDir"], { encoding: "utf8" });
